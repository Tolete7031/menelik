...
# menelik_hash
RUN git clone https://github.com/raptor3um/menelik_hash
RUN cd menelik_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} menelik
RUN useradd -u ${USER_ID} -g menelik -s /bin/bash -m -d /menelik menelik
...
RUN mkdir /menelik-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /menelik-src && \
