...
void static MenelikMiner(const CChainParams& chainparams)
{
    LogPrintf("MenelikMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("menelik-miner");
    ...
    CWallet * pWallet = NULL;
        ...
        if (!EnsureWalletIsAvailable(pWallet, false)) {
            LogPrintf("MenelikMiner -- Wallet not available\n");
        }
    #endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("MenelikMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("MenelikMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("MenelikMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("MenelikMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateMeneliks(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&MenelikMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
