...
public:
    explicit AmountValidator(QObject *parent) :
        QValidator(parent),
        currentUnit(BitcoinUnits::MENEL) {}
...
public:
    ...
    QSize minimumSizeHint() const
    {
        ...
        int h = 0;
        int w = fm.width(BitcoinUnits::format(BitcoinUnits::MENEL, BitcoinUnits::maxMoney(), false, BitcoinUnits::separatorAlways));
        w += 2; // cursor blinking space
        w += GUIUtil::menelikThemeActive() ? 24 : 0; // counteract padding from css
        return QSize(w, h);
    }
