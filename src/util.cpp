...
const char * const BITCOIN_CONF_FILENAME = "menelik.conf";
const char * const BITCOIN_PID_FILENAME = "menelikd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "menelikd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MenelikCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MenelikCore
    // Mac: ~/Library/Application Support/MenelikCore
    // Unix: ~/.menelikcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MenelikCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MenelikCore";
#else
    // Unix
    return pathRet / ".menelikcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty menelik.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Menelik Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Menelik Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Menelik Core developers";
    }
