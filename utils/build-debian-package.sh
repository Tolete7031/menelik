...
PACKAGE_VERSION=$($SRC_PATH/src/menelikd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/menelikd $DEB_BIN
cp $SRC_PATH/src/menelik-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/menelik-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/menelikd.1 $DEB_MAN
cp $SRC_DOC/man/menelik-cli.1 $DEB_MAN
cp $SRC_DOC/man/menelik-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/menelikd.bash-completion $DEB_CMP/menelikd
cp $SRC_PATH/contrib/menelik-cli.bash-completion $DEB_CMP/menelik-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/menelikd.1
gzip --best -n $DEB_MAN/menelik-cli.1
gzip --best -n $DEB_MAN/menelik-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/menelikd $DEB_BIN/menelik-cli
